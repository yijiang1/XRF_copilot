...
# numba @njit in strict nopython mode (numba>=0.60) cannot type 'self' (a
# Python class instance).  These standalone functions hold the compiled logic;
# the corresponding class methods below are thin wrappers.
# ---------------------------------------------------------------------------

@njit
def _g_mask(M_shape, alfa, N):
...
@njit
def _g_radial_mask(M_shape, radial_dis, shape_mask):
...
def _generate_H_jit(tomo3D_shape, theta, atten3D, H_tot, H_zero):
    num = len(theta)
    s = tomo3D_shape
...
class Core(object):
    def __init__(self):
        self.mask3D = None
        self.Atten_slices = None
        self.I_slices = None
        self.mu_all_elem = None

    def load_global_mask(self, Mask3D):
        self.mask3D = Mask3D

    def load_global_atten_slices(self, atten_slices):
        self.Atten_slices = atten_slices

    def load_global_projection_slices(self, i_slices):
        self.I_slices = i_slices


    def maximum_likelihood(self, img2D, p, y, n_iter=10):
    ...
    def mlem_matrix(self, img2D, p, y, n_iter=10):
        '''
        CPU version
        '''
        ...
        b_sum = self.row_sum(p)     #b_sum = np.sum(p, axis=0)
        y[y<0] = 0
        if len(y.shape) == 1:
            y = y.reshape((len(y), 1))
    ...
    def prep_detector_mask3D(self, alfa=15, theta=60, length_maximum=200, fn_save='mask3D.h5'):
        print('Generating detector 3D mask ...')
        mask = {}
        for i in trange(length_maximum, 6, -1):
            mask[f'{i}'] = self.generate_detector_mask(alfa, theta, i)
    ...
    def load_mask3D(self, fn='mask3D.h5'):
        f = h5py.File(fn, 'r')
        keys = f.keys()
        mask3D = {}
        for k in keys:
    ...
    def generate_detector_mask(self, alfa0, theta0, leng0):
        ...
        M1 = self.g_mask((s[0], s[2]), alfa, N1)
        M2 = self.g_mask((s[1], s[2]), theta, N2)
        M1[N1-1,:] = 1
        M1[N1-1,0] = 0
        M2[N2-1,:] = 1
        M2[N2-1,0] = 0
        ...
        M_normal = self.g_radial_mask_approximate(Mask)
    ...
    def g_mask(self, M_shape, alfa, N):
        return _g_mask(M_shape, alfa, N)

    def g_radial_mask(self, M_shape, radial_dis, shape_mask):
        return _g_radial_mask(M_shape, radial_dis, shape_mask)


    def g_radial_mask_approximate(self, Mask):
        s = Mask.shape
        M_normal = np.zeros(s)
    ...
    def get_mask_area_data(self, mask, data, row, col, sli):
    ...
    def fit_surf(self, img, order=2):
        ...
        edge_mask = self.edge_det_mask(img)
        im = edge_mask * img
        ...
        coef = self.polyfit2d(x, y, z, order=order)
        z_fit = self.polyval2d(av, bv, coef)
        im_fit = img / z_fit
    ...
    def write_attenuation(self, elem, data, current_angle, angle_id, file_path='./Angle_prj'):
    ...
    def write_projection(self, mode, elem, data, current_angle, angle_id, file_path='./Angle_prj'):
    ...
    def get_atten_coef(self, elem_type, XEng, em_E):
        '''
        elem_type = ['Zr', 'La', 'Hf']
        XEng = 12
        em_E = [4.6, ]
    ...
    def atten_2D_slice(self, sli, mu_ele):
        mask3D = self.mask3D
        s_mu = mu_ele.shape
        ...
        for i in range(s_mu[1]): # row
            ...
            for j in np.arange(0, s_mu[2]): # column
                cord = self.retrieve_data_mask_cord(s_mu, s_mask, sli, int(i), int(j))
    ...
    def atten_2D_slice_all_elem(self, sli):
        '''
        mu_all_elem: for all elements, shape = (5, 75, 100, 100)
        '''
        mu_all_elem = self.mu_all_elem
        ...
        for i in range(n_row): # row
            ...
            mask = np.asarray(self.mask3D[f'{length}'], dtype='f4')
            s_mask = mask.shape
            for j in np.arange(0, n_col): # column
                cord = self.retrieve_data_mask_cord(s_mu[1:], s_mask, sli, int(i), int(j))
                zs, ze, xs, xe, ys, ye, m_zs, m_ze, m_xs, m_xe, m_ys, m_ye = cord
    ...
    def retrieve_data_mask_cord(self, data_shape, mask_shape, sli, row, col):
        s0 = data_shape
        s = mask_shape
        xs = int(row)
        xe = int(row + s[1])
    ...
    def atten_2D_slice_mpi(self, sli, mu_ele, num_cpu=4):
        ...
        partial_func = partial(self.atten_2D_slice, mu_ele=mu_ele)
        pool = Pool(num_cpu)
    ...
    def atten_voxel(self, idx, row, mu_ele):
        #mask3D = self.mask3D
        s = mu_ele.shape
        ...
        sli = idx // n_sli
        col = idx - sli * n_sli
        atten = self.retrieve_data_mask(mu_ele, row, col, sli, mask)
        return atten


    def atten_2D_row_mpi(self, row, mu_ele, display_flag=True, num_cpu=4):
        ...
        partial_func = partial(self.atten_voxel, row=row, mu_ele=mu_ele)
        pool = Pool(num_cpu)
        #res = pool.map(partial_func, sli)
        res = []
    ...
    @jit
    def retrieve_data_mask(self, data3d, row, col, sli, mask):
        """
    ...
    def cal_atten_3D(self, img4D, cs, param, enable_scale=False, num_cpu=8):
        ...
        # new method (faster)
        res = self.xrf_atten(mu, elem_type, num_cpu)
    ...
    def xrf_atten(self, dict_mu, elem_type, num_cpu=1):
        '''
        ...
        num_cpu:
            ...
            if >1: run using mpi
        '''
        self.mu_all_elem = self.pre_treat(list(dict_mu[k] for k in elem_type))
        max_num_cpu = round(cpu_count() * 0.8)
        if num_cpu == 0 or num_cpu > max_num_cpu:
            num_cpu = max_num_cpu

        s = self.mu_all_elem.shape # (5, 75, 100, 100)
        n_sli = s[1]
        slices = np.arange(n_sli)

        print(f'assembling {len(slices)} slices using {num_cpu:2d} CPUs')
        ts = time.time()
        # partial_func = partial(self.atten_2D_slice_all_elem)
        partial_func = self.atten_2D_slice_all_elem
        pool = Pool(num_cpu)
        res = []
    ...
    def generate_H_jit(self, tomo3D_shape, theta, atten3D, H_tot, H_zero):
        return _generate_H_jit(tomo3D_shape, theta, atten3D, H_tot, H_zero)



    def smooth_filter(self, img, filter_size=3):
        s = img.shape
        img_s = np.zeros(s)
        n = s[0]
        for i in range(n):
    ...
    def absorption_correction(self, sli_id, elem, ref_tomo, angle_list, fpath_atten, n_iter=10,
        ...
        '''
        atten3D = self.Atten_slices[sli_id]
        ...
        img2D[img2D<0] = 0
        res = self.mlem_matrix(img2D, H_jit, I_tot, n_iter)
        if save_tiff:
            if fpath_save[-1] == '/':
    ...
    def cuda_generate_H_single_slice(self, atten_sli, angle_list, H0=None, H_tot=None):
        ...
        if H0 is None or (not cuda.is_cuda_array(H0)):
            # NOTE: float32 halves memory vs float64 but reduces MLEM convergence rate.
            # The CPU path uses float64 H matrices (np.zeros default) and converges in ~8
            # iterations; this GPU float32 path needs ~16 iterations for equivalent accuracy.
            # Switch to float64 here (and in mlem_cuda / new_cuda_array) to match CPU
            # convergence, at the cost of ~2x memory and bandwidth.
    ...
    def recon_astra_sub(self, proj, theta, rot_cen=None, method='FBP_CUDA', num_iter=20):
        if rot_cen is None:
