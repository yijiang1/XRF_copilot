...
def new_cuda_array(shape):
    # NOTE: float32 used throughout GPU MLEM path. This requires ~2x more MLEM iterations
    # than the CPU path (which uses float64 H matrices) to reach equivalent accuracy.
    # See cuda_generate_H_single_slice for details.
...
def mlem_cuda(X, A, B, n_iter=10, blocks=None, threads=256):
    '''
    cuda version of solving X from AX=B
    all the inputs are cuda.array
    ...
    m, n = A.shape
    if blocks is None:
        # kernel_mlem_iter uses cuda.cg.this_grid() (cooperative launch).
        # Max grid = SM_count * max_blocks_per_sm.  Use 2*SM_count as safe upper bound.
        sm_count = cuda.get_current_device().MULTIPROCESSOR_COUNT
        blocks = min(2 * sm_count, 216)
    AX = new_cuda_array((m, 1))
    A_sum = new_cuda_array((1, n))
    B_AX = new_cuda_array((m, 1))
    A_sum_new = new_cuda_array((1, n))
