"""Reconstruction page — composes reconstruction components and sets up polling."""
...
from ..components.recon_parameter_form import create_recon_parameter_form
...
def create_reconstruction_page(api_key: str = ""):
    """Build the reconstruction page.

    Composes reconstruction components into a single-column layout and sets up
    ...
    with ui.column().classes("w-full px-4 gap-4"):
        # Parameter form
        input_elements, valid_params = create_recon_parameter_form(state)

        # Controls
        run_btn, stop_btn = create_recon_control_panel(
            state, api, input_elements, valid_params
        )

        # Reuse the status log component (operates on state.messages)
        _log_area, update_log = create_status_log(state)

        # Progress display adapted for epoch-based progress
        ui.label("Progress:").classes("font-bold mt-4 mb-1")
        progress_info = ui.label("No reconstruction running").classes(
        ...
        def update_progress():
            ...
            if state.is_running or state.current_epoch > 0:
                ...
                progress_info.set_text("No reconstruction running")
    ...
    async def poll_backend():
        """Called every 2 seconds to poll the backend for reconstruction updates."""
        try:
            # Get progress data
            ...
            try:
                ...
                if recon_error and not error_reported["value"]:
                    ...
                    ui.notify(
                        "Reconstruction failed. See status log for details.",
                        type="negative",
                        position="top",
                        timeout=10000,
                    )
