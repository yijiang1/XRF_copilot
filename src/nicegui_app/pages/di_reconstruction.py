"""Di et al. 2017 reconstruction page — composes Di components and sets up polling."""
...
from ..components.di_parameter_form import create_di_parameter_form
...
from ..components.recon_results_gallery import create_recon_results_gallery
...
def create_di_reconstruction_page(api_key: str = ""):
    """Build the Di et al. 2017 reconstruction page.

    Composes Di components into a single-column layout and sets up
    ...
    with ui.column().classes("w-full px-4 gap-4"):
        # Parameter form
        input_elements, valid_params = create_di_parameter_form(state)

        # Controls
        run_btn, stop_btn = create_di_control_panel(
        ...
        # Progress display (outer-epoch based)
        ui.label("Progress (outer epochs):").classes("font-bold mt-4 mb-1")
        progress_info = ui.label("No Di reconstruction running").classes(
        ...
        def update_progress():
            ...
            if state.is_running or state.current_epoch > 0:
                info = (
                    f"Outer Epoch: {state.current_epoch}/{state.total_epochs}  |  "
                    f"Completion: {pct:.1f}%"
                )
                progress_info.set_text(info)
            else:
                progress_info.set_text("No Di reconstruction running")

        update_results = create_recon_results_gallery(state)
    ...
    async def poll_backend():
        """Called every 2 seconds to poll the backend for Di reconstruction updates."""
        try:
            # Get progress data
            progress_data = await api.get_di_recon_progress()
            state.is_running = progress_data.get("is_running", False)
            state.current_epoch = progress_data.get("current_epoch", 0)
            state.total_epochs = progress_data.get("total_epochs", 0)
            if state.total_epochs > 0:
                state.progress_percent = (
                    state.current_epoch / state.total_epochs * 100.0
                )
            else:
            ...
            # Check for errors
            try:
                recon_status = await api.get_di_recon_status()
                recon_error = recon_status.get("error")
                if recon_error and not error_reported["value"]:
                    error_reported["value"] = True
                    state.messages = append_to_message_list(
                        state.messages,
                        f"Di reconstruction error: {recon_error}",
                        level="ERROR",
                    )
                    ui.notify(
                        "Di reconstruction failed. See status log for details.",
                        type="negative",
                        position="top",
                        timeout=10000,
                    )
            ...
            try:
                ...
                if recon_file and not results_data.get("error"):
                    state.recon_file = recon_file
                    state.results_ready = True
                    if not state.results_displayed:
                        ui.notify(
            ...
            if state.is_running:
                try:
                    ...
                    worker_logs = worker_data.get("logs", [])
