...
def di_run_endpoint(session_manager, di_setup_output):
    """Register the run Di reconstruction endpoint (session-based)."""

    @router.post("/run_di_reconstruction/")
    async def run_di_reconstruction():
        try:
            params = di_setup_output.get("params")
            if not params:
                raise HTTPException(
                    status_code=400,
            ...
            session = session_manager.create_session("Wendy", params)
            ...
            process.start()
            logger.info(
                f"Di reconstruction worker started — session {session.session_id}, PID {process.pid}"
            )
            ...
            logger.error(f"Error starting Di reconstruction: {e}")
